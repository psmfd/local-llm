...
- **Model (single workhorse, ADR-009 lineup, ADR-010 quant):** **`coding-workhorse`** — `mlx-community/GLM-4.7-Flash-6bit` (`Glm4MoeLiteForCausalLM`, MoE ~3B active, 202K ctx, MLA KV compression), ~24 GB. **Pinned, sole resident model.** A verified text-only coder build (no `vision_config` → batched LLM engine, no override), tool-call-verified on oMLX; quality parity with the 8-bit measured on-host (ADR-010). High-fidelity work routes to the **cloud frontier**, not a local tier. Retired models (`GLM-4.7-Flash-8bit` — kept on disk as the primary inactive fallback, renamed to alias `workhorse-8b` — plus ADR-006's `Qwen3-Coder-30B-A3B-Instruct-MLX-8bit` (secondary fallback) and `Qwen3-Coder-Next-MLX-4bit`) are never downloaded or served; setup unpins them if a prior install left them pinned.
- **Serving flags:** `--host 127.0.0.1` (explicit loopback pin), port `8000`, `--memory-guard-gb 90` (replaces the removed `--max-process-memory`), `--paged-ssd-cache-dir ~/.omlx/cache`, `--paged-ssd-cache-max-size 50GB` (the upstream default is 100 GB — past the 90 GB free-disk preflight budget), `--hot-cache-max-size 24GB` (oMLX accepts absolute sizes and percentages; an absolute value is pinned for a deterministic footprint), `--max-concurrent-requests 4` (ADR-012's large-context mark — ADR-010's mark of 8 was measured at ~16K contexts and oversubscribes the ~83K-token shared KV pool at real 25–45K agentic streams; at 4, matching the pi subagent spawn cap, excess queues at admission), `--api-key` from the 0600 file. The pi provider advertises `contextWindow 76800` (the measured prefill-guard boundary, ADR-011) and `maxTokens 8192` (ADR-012).
- **Metal wired limit:** raise `iogpu.wired_limit_mb` to ~96 GB (98304) so Metal can wire the workhorse (~24 GB) with maximal KV/prefix-cache headroom; persist across reboot via a LaunchDaemon (needs sudo).
